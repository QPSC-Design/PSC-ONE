                ...
                S_START: begin
                    ...
                    if (!mode_sv32 || (priv_mode == PRIV_M)) begin
                        ...
                        if(cache_hit) begin    // PTEキャッシュあり
                            ...
                            //mem_addr <= ({10'b0, root_ppn} << 12) + ({22'b0, vpn1} << 2);
                            mem_addr <= {root_ppn, 12'b0} + {vpn1, 2'b00};
                            if(mem_req_ready)
                                state    <= S_L1_REQ;
                        end
                    end
                ...
                S_L1_CHECK: begin
                    ...
                    if (!pte_valid(pte_l1)) begin
                        ...
                        if (|pte_l1[19:10]) begin
                            ...
                            paddr <= {pte_l1[31:20], vpn0, page_off};
                            state <= S_DONE;
                        end
                    end else begin
                        // ポインタPTE → L0
                        if (l0_cache_hit) begin
                            ...
                            //mem_addr <= ({10'b0, pte_l1[31:10]} << 12) + ({22'b0, vpn0} << 2);
                            mem_addr <= {pte_l1[31:10], 12'b0} + {vpn0, 2'b00};
                            if(mem_req_ready)
                                state    <= S_L0_REQ;
                        end
