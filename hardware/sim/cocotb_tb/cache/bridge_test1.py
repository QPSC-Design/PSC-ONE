# ===============================================================
#  NISHIHARU simulate_16bit_to_128bit_axi_bridge TEST (FINAL)
# ===============================================================
import cocotb
    ...
    while True:
        ...
        if state["reading"]:
            if state["read_idx"] < 8:
                ...
                dut.m_axi_rlast.value  = (state["read_idx"] == 7)
            else:
                dut.m_axi_rvalid.value = 0
        else:
            dut.m_axi_rvalid.value = 0
    ...
    while True:
        ...
        if int(dut.m_axi_wvalid.value):
            ...
            if idx < 8:
                ...
                dut._log.info(f"W[{idx}] = 0x{data:04x}")
                state["write_idx"] += 1
        ...
        if state["reading"] and int(dut.m_axi_rready.value):
            ...
            if idx < 8:
                ...
                dut._log.info(f"R[{idx}] = 0x{data:04x}")
...
async def axi_test1(dut):
    ...
    dut._log.info("==== AXI bridge test start ====")
